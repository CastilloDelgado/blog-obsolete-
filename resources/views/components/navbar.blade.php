<nav class="md:flex md:justify-between md:items-center bg-gray-100 p-5">
  <div>
    <a href="/">
      <div class="flex flex-row ">
        <p class="text-2xl font-bold">Life & life music</p>
        <small class="font-light font-sans ml-1">(blog)</small>
      </div>
      {{-- <img src="/images/logo.svg" alt="Laracasts Logo" width="165" height="16"> --}}
    </a>
  </div>

  <div class="mt-8 md:mt-0 flex items-center">

    <a href="/register" class="text-sm font-sans hover:underline">Registrate</a>
    <a href="/login" class="text-sm ml-4 font-sans hover:underline">Iniciar Sesión</a>

    <a href="#newsletter" class="font-sans bg-black ml-3 rounded text-sm font-semibold text-white py-2 px-3">
      Newsletter
    </a>
  </div>
</nav>